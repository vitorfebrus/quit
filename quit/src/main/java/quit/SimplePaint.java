...
/**
 * A simple applet where the user can sketch curves in a variety of colors. A
 * color palette is shown along the right edge of the applet. The user can
 * select a drawing color by clicking on a color in the palette. Under the
 * colors is a "Clear button" that the user can press to clear the sketch. The
 * user draws by clicking and dragging in a large white area that occupies most
 * of the applet. The user's drawing is not persistent. It is lost whenever the
 * applet is repainted for any reason.
 * <p>
 * The drawing that is done in this example violates the rule that all drawing
 * should be done in the paintComponent() method. Although it works, it is NOT
 * good style.
 * <p>
 * This class also contains a main program, and it can be run as a stand-alone
 * application that has exactly the same functionality as the applet.
 */
public class SimplePaint extends JFrame {

	/**
	 * The main routine opens a window that displays a drawing area and color
	 * palette. This main routine allows this class to be run as a stand-alone
	 * application as well as as an applet. The main routine has nothing to do with
	 * the function of this class as an applet.
	 */
	...
	/**
	 * The init method of the applet simply creates a SimplePaintPanel and uses it
	 * as the content pane of the applet. All the work is done by the
	 * SimplePaintPanel.
	 */
	public void init() {
	...
	/**
	 * A simple paint panel contains a large white drawing surface where the user
	 * can draw curves and a color palette that the user can click to select the
	 * color to be used for drawing. When this class is used as an applet, the
	 * content pane of the applet is a SimplePaintPanel. When this class is run as a
	 * standalone application, the content pane is a SimplePaintPanel. All the real
	 * work is done in the SimplePaintPanel class.
	 */
	public static class SimplePaintPanel extends JPanel implements MouseListener, MouseMotionListener {
		...
		/**
		 * Constructor for SimplePaintPanel class sets the background color to be white
		 * and sets it to listen for mouse events on itself.
		 */
		SimplePaintPanel() {
		...
		/**
		 * Draw the contents of the panel. Since no information is saved about what the
		 * user has drawn, the user's drawing is erased whenever this routine is called.
		 */
		public void paintComponent(Graphics g) {
			...
			/*
			 * Draw a 3-pixel border around the applet in gray. This has to be done by
			...
			/*
			 * Draw a 56-pixel wide gray rectangle along the right edge of the applet. The
		 ...
		 * palette at a point with y-coordinate y. (Note that I can't just call repaint
		 * and redraw the whole panel, since that would erase the user's drawing!)
		 */
		private void changeColor(int y) {
			...
			/*
			 * Remove the hilite from the current color, by drawing over it in gray. Then
			 * change the current drawing color and draw a hilite around the new drawing
			 * color.
			 */
		...
		private void setUpDrawingGraphics() {
			...
			graphicsForDrawing.setStroke(new BasicStroke(5));
		...
		/**
		 * This is called when the user presses the mouse anywhere in the applet. There
		...
		/**
		 * Called whenever the user releases the mouse button. If the user was drawing a
		 * curve, the curve is done, so we should set drawing to false and get rid of
